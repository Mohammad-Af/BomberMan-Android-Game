...
public class Monster3 extends Monster{
    ...
    public Monster3(int x, int y) {
        super(x, y);
        speed=60;
        lastArrayImage= Images.monster3WU;
    }
        ...
        if (imageCounter % speed == 0) {
            if (alive) {
                ...
                if (!BomberMan.alive) {                       //vaghti bomberman mord random harekat kone
                    ...
                    continueRandomMove(board);                                         //vaghti donbale bombermane va tu ye khune gir mikone bayad 10 harekate random anjam bde
                    randomMoves++;
                    if (randomMoves == 20)
                        randomMoves = 0;
                }
    ...
    private boolean makeMove(Board board) {
        ...
        if (location.y % Board.cellsSize == 0 && location.x > xBomberman && rand < 0.5) {
            ...
            if (!board.blockInLocation(x,y) && !board.wallInLocation(x,y) && !board.bombInLocation(x, y)) {
                lastArrayImage = Images.monster3WL;
            ...
            int y = location.y;
            if (!board.blockInLocation(x,y) && !board.wallInLocation(x,y) && !board.bombInLocation(x, y)) {
                lastArrayImage = Images.monster3WR;
                location = new Point(location.x + movementSize, location.y);
            ...
            if (!board.blockInLocation(x,y) && !board.wallInLocation(x,y) && !board.bombInLocation(x, y)) {
                lastArrayImage = Images.monster3WD;
    ...
    private void continueRandomMove(Board board) {
